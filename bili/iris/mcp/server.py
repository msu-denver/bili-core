...
runs as*, so a valid token is evidence of file access rather than evidence of
being the intended caller: it keeps out other users on the host, and it does
not keep out other processes of the same user.

Every request therefore satisfies a second, independent check.  Its
connection must be owned by the spawned subprocess or one of that
subprocess's descendants, decided in
:mod:`~bili.iris.mcp.peer_identity`.  A request with a valid token from a
process outside that tree is refused with ``403 Forbidden``; a request
without the token is refused with ``401 Unauthorized``.  The server denies
everything until :meth:`EphemeralMcpServer.authorize_subprocess` records the
process that was spawned, so the window between "listening" and "spawned" is
closed rather than open.

The residual, stated plainly
-----------------------------
This raises the bar from "holds the secret" to "is inside the spawned process
tree".  It does not make that tree a security boundary between programs run
by one user: a same-user attacker who can attach to, or inject into, the
spawned process is indistinguishable from it by construction, and same-user
isolation is inherently weak on a workstation.  What it refuses is the
opportunistic case the token cannot: another program of the same user that
has read the token and calls the server directly.
...
class _TokenAuthMiddleware:  # pylint: disable=too-few-public-methods
    """ASGI middleware enforcing Bearer-token auth *and* caller identity.

    Every HTTP request to the wrapped application must satisfy two independent
    checks, in this order:

    1. It carries ``Authorization: Bearer <token>``, where ``<token>`` is the
       cryptographically-random value generated by
       :class:`EphemeralMcpServer` for this call.  Failing this is
       ``401 Unauthorized``.
    2. Its connection is owned by the spawned subprocess or one of that
       subprocess's descendants, per
       :meth:`~bili.iris.mcp.peer_identity.PeerAuthorization.permits`.
       Failing this is ``403 Forbidden``.

    The second check exists because the first cannot stand alone: the token
    reaches the subprocess through a file or an environment variable, so every
    process running as the same user can read it, and a valid token is
    therefore evidence of file access rather than of being the intended
    caller.  The two statuses are kept distinct so a refusal says which
    property failed; a wrong token and a token used by the wrong process are
    ...
    :param authorization: The shared
        ...
        everything until the subprocess has been spawned and authorized.
    :param server_port: The port this server listens on, used to identify the
        inbound connection from the subtree's side.
    """
    ...
    async def __call__(self, scope: Dict, receive: Any, send: Any) -> None:
        ...
        if not secrets.compare_digest(auth_value, self._bearer_value):
            LOGGER.debug(
        ...
        # The token is right. Is the caller the process this server was
        # started for?  A missing or malformed client address cannot be
        # attributed to the authorized subtree, so it is refused rather than
        # waved through: an unidentifiable peer is exactly the case this
        # check exists for.
        client = scope.get("client")
        peer_port = client[1] if client and len(client) > 1 else None
        if peer_port is None:
            LOGGER.warning(
                "EphemeralMcpServer: rejected request carrying a valid token "
                "with no client address on the ASGI scope; the caller cannot "
                "be attributed to the authorized subprocess."
            )
            await _send_json_error(send, 403, "Forbidden")
            return

        if not self._authorization.permits(peer_port, self._server_port):
            await _send_json_error(send, 403, "Forbidden")
            return
...
async def _send_json_error(send: Any, status: int, message: str) -> None:
    """Send a minimal JSON error response and close the exchange.

    :param send: The ASGI send callable.
    :param status: HTTP status code.
    :param message: Value for the response body's ``error`` key.
    """
...
class EphemeralMcpServer:  # pylint: disable=too-many-instance-attributes
    ...
    7. Revokes the process authorization, so a connection cannot outlive the
       grant that permitted it.
    8. Sets ``uvicorn.Server.should_exit = True`` to signal shutdown.
    9. Joins the background thread with a :data:`_SHUTDOWN_JOIN_TIMEOUT` grace
       period.  The thread is a daemon, so the process does not hang if it
       fails to exit promptly.

    The server starts in a **deny-all** state.  A correct token is necessary
    and not sufficient: until :meth:`authorize_subprocess` records the process
    that was spawned, every authenticated request is refused with ``403``.
    Callers that spawn their own subprocess (rather than going through
    :func:`build_mcp_node`, which does this for them) must call it::

        server = EphemeralMcpServer(tools)
        with server as handle:
            result = injector.inject(command=cmd, handle=handle)
            proc = subprocess.Popen(result.augmented_command, ...)
            server.authorize_subprocess(proc.pid)
            ...

    Authorizing after the spawn rather than before is forced by the ordering:
    the server must already be listening for the injector to have a URL to
    ...
    def __init__(
        ...
        self._authorization = PeerAuthorization()
        # Set in __enter__:
    ...
    def authorize_subprocess(self, pid: int) -> ProcessIdentity:
        """Permit *pid* and its descendants to call this server.

        Call immediately after spawning the subprocess the server exists to
        serve.  Until this is called nothing is permitted, which is why the
        window between "listening" and "spawned" is not an exposure.

        :param pid: The spawned subprocess's process id.
        :returns: The captured :class:`ProcessIdentity` (PID plus creation
            time, so a recycled PID cannot inherit the grant).
        :raises psutil.NoSuchProcess: If the subprocess has already exited.
        """
        return self._authorization.authorize_subprocess(pid)
    ...
    def _stop_server(self) -> None:
        ...
        # Revoke first. Shutdown is not instantaneous, so an in-flight request
        # must not be served against a grant the caller has already released.
...
def build_mcp_node(
    ...
    def _node(state: Dict) -> Dict:  # pylint: disable=too-many-locals
        ...
        with server as handle:
            ...
            # Popen rather than subprocess.run: the server denies every request
            # until the process it was started for is known, and that PID does
            # not exist until the spawn returns.  subprocess.run never exposes
            # it, so the authorization could not be granted at all.
            try:
                ...
                # Authorize before writing the prompt: the CLI may connect as
                # soon as it starts, and a tool call that lands before the
                # grant is refused rather than queued.
                server.authorize_subprocess(proc.pid)
                try:
                    ...
                    # communicate() does not kill on timeout the way
                    # subprocess.run does; without this the child outlives the
                    # server that was serving it.
                    proc.kill()
                    proc.communicate()
                    raise CliLLMError(
        ...
        if returncode != 0:
            ...
            raise CliLLMError(
                f"CLI subprocess exited with code {returncode}: "
