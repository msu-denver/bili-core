"""Tests for bili.aegis.attacks.injector (AttackInjector)."""
...
from bili.aegis.attacks.strategies.mid_execution import run_with_mid_execution_injection
from bili.aegis.tests.conftest import _agent
...
def test_blocking_true_returns_with_completed_at(tmp_path):
    ...
    with patch("bili.aether.runtime.executor.MASExecutor") as mock_executor_cls:
        ...
        with patch(
            ...
            with patch("bili.aegis.attacks.injector.AttackLogger"):
                result = injector.inject_attack(
                    agent_id="agent_a",
                    attack_type=AttackType.PROMPT_INJECTION,
                    payload=_PAYLOAD,
...
def test_pre_execution_dispatches_to_correct_strategy(
    ...
    strategy_module = "bili.aegis.attacks.strategies.pre_execution"
...
def test_mid_execution_raises_runtime_error_on_wrong_node(tmp_path):
    ...
    with patch(
        "bili.aegis.attacks.strategies.mid_execution.run_with_mid_execution_injection"
    ) as mock_mid:
