...
Default log path: ``bili/aegis/attacks/logs/attack_log.ndjson``

Usage::

    from pathlib import Path
    from bili.aegis.attacks.logger import AttackLogger
...
from bili.aegis.attacks.models import AttackResult
...
class AttackLogger:  # pylint: disable=too-few-public-methods
    ...
    Args:
        log_path: Path to the NDJSON log file.  Parent directories are
            ...
            ``bili/aegis/attacks/logs/attack_log.ndjson``.
    """
    ...
    def log(self, result: AttackResult) -> None:
        ...
        The file is opened in append mode (``"a"``) so existing records are
        never overwritten.
