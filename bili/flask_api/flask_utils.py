...
def handle_agent_prompt(user, conversation_agent, prompt, conversation_id=None):
    """
    Processes a user prompt using a conversation agent by associating the prompt
    with a specific thread ID based on the user's email and optional conversation ID.
    The function converts the prompt into a usable format for the conversation agent
    and retrieves the generated response, returning it in JSON format.

    Multi-conversation support:
        - If conversation_id is provided: thread_id = "{email}_{conversation_id}"
        - If conversation_id is None: thread_id = "{email}" (backward compatible)
    ...
    :type prompt: str
    :param conversation_id: Optional conversation identifier for multi-conversation
        support. When provided, enables users to maintain multiple separate
    ...
    if conversation_id:
        ...
        # Backward compatible: use email only when no conversation_id provided
        thread_id = email
