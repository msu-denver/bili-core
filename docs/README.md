...
| **Multimodal Input** | Attach an image to a turn with `bili.iris.multimodal`; per-model `input_modalities` in the catalog let `load_model(..., required_input_modalities=["image"])` refuse a text-only model at selection, and text-only transports refuse an image part rather than dropping it |
