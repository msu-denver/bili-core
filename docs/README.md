...
| Feature | Description |
|---------|-------------|
| **118 LLM Configurations** | Pre-configured models across 18 provider types — API, CLI, and local (via `bili.iris.config`) |
...
| **Multimodal Input** | Attach an image to a turn with `bili.iris.multimodal`; per-model `input_modalities` in the catalog let `load_model(..., required_input_modalities=["image"])` refuse a text-only model at selection. A message-based provider is handed the bytes; a CLI provider materializes the image in its own working directory and points the harness at it, reporting the weaker `offered_by_path` delivery kind. A transport with no route for an image refuses it rather than dropping it |
